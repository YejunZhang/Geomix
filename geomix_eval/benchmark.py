...
from geomix.data.dataset_loaders import init_data_loader, init_eval_data_loader
from geomix.utils.evaluator import MatcherEvaluator, summarize_metrics
from geomix.utils.logger import get_logger

logger = get_logger(level="INFO", name="geomix_eval")
...
def eval_model(
    ...
    for covis_k in covis_k_nums:
        ...
        evaluator = MatcherEvaluator(
            vismatch=vismatch, ckpt_path=ckpt_path, oracle=oracle, sc_thres=sc_thres
        )
...
def run_benchmark(
    ...
    data_loader = init_eval_data_loader(config, split=split, batch=16, p2d_type=p2d_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--p2d_type", type=str, choices=["sift", "superpoint", "disk", "dedode", "r2d2"], default="sift"
    )
    ...
    parser.add_argument("--sc_thres", type=float, default=0.5,
                        help="Outlier rejection threshold for cls-style matchers (e.g., 0 disables it).")
