...
from geomix.utils.metrics import summarize_metrics
...
def plot_orate_ablation_curves(model_results, model_colors, max_orates, auc_thresholds):
    ...
    # Define markers for each model to make lines more distinguishable
    markers = ['o', 's', '^', 'D', 'v']
    marker_dict = {name: markers[i] for i, name in enumerate(model_colors.keys())}
    ...
    for idx, th in enumerate(auc_thresholds):
        ...
        for name, color in model_colors.items():
            ...
            plt.plot(xrange, aucs, color=color.value, ls="-", linewidth=3, 
                    marker=marker_dict[name], markersize=10, markeredgewidth=2,
    ...
    plt.legend(fontsize=24, bbox_to_anchor=(1.1, 0.73, 0, 0), frameon=True, 
