...
class BaseDataset(data.Dataset):
    ...
    def _load_scene_data(self, dataset_conf: Namespace) -> None:
        ...
        if not os.path.exists(scene3d_file):
            scene3d_file = os.path.join(data_dir, "scene_points3d", "all.npy")
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        for offset in range(n):
            ...
            try:
                ...
                continue
            if len(data["pts3d"]) > 0 and len(data["pts2d"]) > 0:
                return data
...
class SIFTDataset(BaseDataset):
    default_config = {**BaseDataset.default_config, "p2d_type": "sift", "topk": 1}


class SuperPointDataset(BaseDataset):
    default_config = {**BaseDataset.default_config, "p2d_type": "superpoint", "topk": 3}


class DISKDataset(BaseDataset):
    default_config = {**BaseDataset.default_config, "p2d_type": "disk", "topk": 3}

class DedodeDataset(BaseDataset):
    default_config = {**BaseDataset.default_config, "p2d_type": "dedode", "topk": 3}

class R2D2Dataset(BaseDataset):
    default_config = {**BaseDataset.default_config, "p2d_type": "r2d2", "topk": 3}
