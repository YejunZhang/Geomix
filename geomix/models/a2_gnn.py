...
class OTMatcher(nn.Module):
    def __init__(
        ...
        self.attention = SCAtt2D3D(att_layers, num_registers=num_registers) if att_layers else None
    ...
    def encode_pts(
        ...
        desc2d = self.kp2d_enc(pts2d, idx2d) + self.color_enc(color2d, idx2d) #+ 0.5 * self.kp2d_enc(pts2d, idx2d) * self.color_enc(color2d, idx2d)
        desc3d = self.kp3d_enc(pts3d, idx3d) + self.color_enc(color3d, idx3d) #+ 0.5 * self.kp3d_enc(pts3d, idx3d) * self.color_enc(color3d, idx3d)
