...
def compute_metrics_sample(
    ...
    # Matcher-only metrics (no PnP). matches_est_, matches_gt_ are bool (n3d, n2d).
    me = matches_est_.bool().to(matches_gt_.device)
    mg = matches_gt_.bool()
    tp = int((me & mg).sum())
    n_pred = int(me.sum())
    n_gt = int(mg.sum())
    precision = tp / n_pred if n_pred > 0 else 0.0   # = matching accuracy / inlier ratio
    recall    = tp / n_gt   if n_gt   > 0 else 0.0   # = recall rate
    f1        = (2 * precision * recall / (precision + recall)) if (precision + recall) > 0 else 0.0
    metrics["match_precision"].append(precision)
    metrics["match_recall"].append(recall)
    metrics["match_f1"].append(f1)
    metrics["n_gt_matches"].append(n_gt)
    metrics["n_tp"].append(tp)

    # Pose estimation
    # i3d, i2d = torch.where(matches_est_)
    # inl = torch.sum( torch.any(i3d.cpu().unsqueeze(1) == i3d_gt.unsqueeze(0), dim=1) )

    # Move indices to CPU to match the device of pts2d and pts3d
    i2d_cpu = i2d.cpu()
    i3d_cpu = i3d.cpu()
    ...
    if not pose_res:
        ...
        metrics["R_est"].append(np.eye(3))
        metrics["t_est"].append(np.zeros(3))
    else:
        R, t, inliers = pose_res
        R_err, t_err = pose_err(R, R_gt, t, t_gt)
        # Check if R and t are torch tensors or numpy arrays
        if isinstance(R, torch.Tensor):
            metrics["R_est"].append(R.cpu().numpy())
        else:
            metrics["R_est"].append(R)
        if isinstance(t, torch.Tensor):
            metrics["t_est"].append(t.cpu().numpy())
        else:
            metrics["t_est"].append(t)
    metrics["R_err"].append(R_err)
    metrics["t_err"].append(t_err)
    metrics["n_matches"].append(len(i3d_cpu))
...
def summarize_metrics(
    ...
    # Matcher-only metrics (GT match-matrix based)
    def _mean(key, default=-1.0):
        v = getattr(metrics, key, [])
        v = np.asarray(v, dtype=np.float32)
        v = v[v >= 0]
        return float(v.mean()) if len(v) else default

    prec = _mean("match_precision")
    rec  = _mean("match_recall")
    f1   = _mean("match_f1")
    ngt  = _mean("n_gt_matches")
    ntp  = _mean("n_tp")
    _logger.info(
        f"Matcher-only: precision/inlier-ratio={100*prec:.2f}% "
        f"recall={100*rec:.2f}% F1={100*f1:.2f}% "
        f"mean_n_gt={ngt:.1f} mean_n_tp={ntp:.1f}"
    )
